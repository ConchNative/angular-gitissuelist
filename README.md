...
- Convert the static phone list to dynamic by:
  - Creating a `gitIssueListController` controller.
...
- Add a simple unit test for the `gitIssueListController` controller to show how to write tests and
  run them using Karma.

### step-3 _Components_

- Introduce components.
- Combine the controller and the template into a reusable, isolated `gitIssueList` component.
- Refactor the application and tests to use the `gitIssueList` component.
...
- Create a phone list route (`/phones`):
  - Map `/phones` to the existing `gitIssueList` component.
...
app/                     --> all the source code of the app (along with unit tests)
  ...
  git-issue-list/...         --> files for the `gitIssueList` module, including JS source code, HTML templates, specs
